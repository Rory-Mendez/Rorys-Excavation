...
**v0.5.0** delivers full vein excavation: hold the configured key (default: Grave/tilde `` ` ``) while breaking a block and all face-adjacent connected blocks of the same type are broken in one action, up to the `maxBlocks` limit (default 64). Without the key only detection and BFS count are reported — no world changes.
