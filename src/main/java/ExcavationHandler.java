...
public class ExcavationHandler implements ITickHandler {
    ...
    public void tickStart(EnumSet<TickType> type, Object... tickData) {
        ...
        if (prevBlockId != 0 && mc.f.a(prevX, prevY, prevZ) == 0) {
            ...
            // ── Blacklist gate ────────────────────────────────────────────────
            // Blacklisted block IDs are never excavated automatically.
            // Manual breaking (which already happened) is unaffected.
            if (config.getBlacklist().contains(prevBlockId)) {
                clearPrev();
                // fall through to step 2 (update tracking for the next tick)
            } else {

                // ── Debug message (gated) ─────────────────────────────────────
                // aiy.a(String) = GuiIngame.printChatMessage (confirmed by javap).
                // Only emitted when debugMessages=true; never shown during normal play.
                if (config.isDebugMessages() && mc.w != null) {
